...
- **Persistence.** Every change writes to `localStorage` immediately, through a guarded accessor: in a sandboxed iframe even reading `window.localStorage` throws a `SecurityError`, so an unguarded access at module scope stopped the whole script before first render. The accessor probes once, exposes `available`, and degrades to in-memory state with the rail reporting that nothing is being saved. When a file handle is connected, a debounced write goes to the file 700 ms later. The handle is stored in IndexedDB so the file reconnects after reload; the browser may require one click to re-grant permission. Export and import cover browsers without file access.
...
- **Timeline.** The window is one fiscal year, chosen by `ui.tlFy` (empty means the current year and follows the calendar; a year number pins it; `all` restores the old auto range). Bars overlapping the window are clipped to it and marked, so a long-running idea shows where it crosses the boundary instead of being dropped or overflowing; ideas entirely outside are listed separately. Quarter scale measures the real scroll pane (inserted empty first, so its `clientWidth` already accounts for content padding and its own borders) and derives pixels per day so the year fits exactly with no horizontal scroll; month scale keeps a fixed 4 px per day and scrolls. A resize re-renders so the fitted scale stays correct. Pointer events move a bar or resize either edge; the change commits as a history entry on release. A click without movement opens the idea.
