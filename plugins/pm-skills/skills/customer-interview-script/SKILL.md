...
1. Pull top assumptions from `identify-assumptions`; write 2-3 must-answer questions and pick the interview type.
2. Customize `assets/interview_script_template.md` and pull type-specific prompts from `assets/question_bank.md`.
3. Run the 5-phase script (rapport first, stories in the middle, contradictions at the end); debrief within 30 minutes; hand the transcript to `interview-synthesis`.
...
**Out of scope:** interview synthesis and theme clustering (`interview-synthesis`); assumption mapping before interviews (`identify-assumptions`); survey design and quantitative research; usability testing protocols (task-based, not story-based); recruiting operations (panel sourcing, screening, incentives).
...
|-------------|-----------|------------|
| `identify-assumptions` | Receives from | Top assumptions become the must-answer questions for each interview |
| `interview-synthesis` | Feeds into | Transcripts and debrief notes are the input for theme clustering |
| `brainstorm-ideas` | Bidirectional | Pre-interview hypotheses; post-interview ideas seeded by themes |
| `brainstorm-experiments` | Feeds into | Validated pains become experiment hypotheses |
| `jtbd-workshop` | Complementary | Switch interviews use this script structure as their foundation |
| `value-proposition-canvas` | Feeds into | Jobs, pains, and gains captured in stories populate the Customer Profile |
| `create-prd` | Feeds into | Direct quotes strengthen PRD Background and Market Segments |
