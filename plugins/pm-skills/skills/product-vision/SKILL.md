...
**Out of scope:** mission statement writing; strategy document construction (`c-level-advisor/`); brand positioning and messaging (`marketing/`); Working Backwards PR/FAQ (`prfaq`); NSM definition (`north-star-metric`); OKR drafting (`brainstorm-okrs`).
...
|-------------|-----------|------------|
| `value-proposition-canvas` | Receives from | Customer Profile (jobs, pains, gains) feeds Pichler Board "Needs" block |
| `jtbd-workshop` | Receives from | Job hierarchy and top outcomes inform the vision's customer + outcome |
| `customer-interview-script` | Receives from | Verbatim customer language sharpens vision phrasing |
| `north-star-metric` | Feeds into | The NSM derives from the vision -- the vision's outcome becomes the NSM input metric tree root |
| `outcome-roadmap` | Feeds into | The roadmap delivers the vision; every roadmap theme should trace back |
| `brainstorm-okrs` | Feeds into | OKRs serve the vision -- each quarterly objective should advance one vision pillar |
| `prfaq` | Complementary | Working Backwards PR is one expression of the vision; the FAQ stress-tests it |
| `create-prd` | Feeds into | PRDs explicitly reference the vision in Section 3 (Background) |
| `roadmap-communication` | Feeds into | Vision is the opening frame of every exec/customer roadmap presentation |
