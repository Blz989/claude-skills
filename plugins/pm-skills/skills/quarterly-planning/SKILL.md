...
**Out of scope:** writing individual OKRs (`brainstorm-okrs`); roadmap construction (`outcome-roadmap`); sprint-level planning (`scrum-master/`); multi-quarter / annual strategy (`c-level-advisor/`); performance management (`career/`); portfolio management (`program-manager/`).
...
|-------------|-----------|------------|
| `brainstorm-okrs` | Bidirectional | Quarterly cycle uses OKRs; OKR drafting is a kickoff sub-skill |
| `outcome-roadmap` | Bidirectional | Quarterly OKRs become Now-horizon roadmap; roadmap themes inform candidate outcomes |
| `north-star-metric` | Receives from | NSM is the long-term metric; quarterly KRs are short-term moves on it |
| `product-vision` | Receives from | Every Objective must trace to a vision pillar |
| `dependency-map` | Receives from | Pre-quarter dependency mapping prevents week-6 surprises |
| `status-update-generator` | Feeds into | Weekly Wodtke rhythm produces input for exec status updates |
| `cycle-time-analyzer` | Receives from | Flow metrics inform capacity assessment |
| `customer-interview-script` | Receives from | Recent customer signal feeds the strategy review |
| `jtbd-workshop` | Receives from | Top desired outcomes seed the outcome candidate list |
...
| `pm-onboarding` | Complementary | New PMs use this skill in their 30-60-90 plan to ramp on team cadence |
