...
**Out of Scope:** pulling raw event data (input is JSON — use Amplitude/Mixpanel/PostHog/Looker); statistical significance testing (`brainstorm-experiments`, data-analytics); cohort-retention analysis; channel attribution (marketing tools); building the UX/A/B test; forecasting.
...
| `brainstorm-experiments` | Pairs with | Each funnel leak suggests testable experiments to plug it |
| `identify-assumptions` | Pairs with | "Users will complete step 3 if we shorten it" is an assumption to validate |
