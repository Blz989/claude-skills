...
**In scope:** 2h/4h/8h formats; switch interview script and four-anchor timeline; forces-of-progress mapping; Ulwick outcome statements with Importance x Satisfaction scoring; Klement situation-motivation-outcome decomposition (bridge to `job-stories`); pre-work email and participant brief.

**Out of scope:** writing individual backlog stories (`job-stories`); quantitative ODI surveying at scale; recording/transcription tooling; recruiting operations; personas (JTBD is segment-by-job, not persona-by-demographic).
...
|-------------|-----------|------------|
| `customer-interview-script` | Receives from | Switch interviews use the base interview script structure |
| `interview-synthesis` | Receives from | Synthesized themes seed the snippet wall on workshop day |
| `identify-assumptions` | Feeds into | Top outcomes become assumptions to test |
| `brainstorm-experiments` | Feeds into | Forces-of-progress weaknesses become experiment hypotheses |
| `value-proposition-canvas` | Feeds into | Jobs, pains, gains populate the Customer Profile |
| `job-stories` | Feeds into | Job + outcome decomposition becomes When/Want/So backlog stories |
| `create-prd` | Feeds into | Job hierarchy populates PRD Section 5; outcomes populate Section 6 |
| `outcome-roadmap` | Feeds into | Top desired outcomes become roadmap themes |
| `north-star-metric` | Feeds into | The highest-priority outcome often becomes the input metric tree root |
