...
**Out of Scope:** statistical power analysis or sample size calculation (use dedicated A/B test platforms); experiment infrastructure setup (feature flags, analytics instrumentation); running the actual experiment (this skill designs, not executes); long-term product strategy or roadmap decisions (`reference/outcome-roadmap/`).
...
| `reference/create-prd/` | Feeds into | Validated hypotheses become PRD assumptions with evidence |
| `reference/brainstorm-okrs/` | Feeds into | Experiment metrics may become OKR key results |
| `reference/outcome-roadmap/` | Feeds into | Experiment outcomes inform Now/Next/Later roadmap placement |
