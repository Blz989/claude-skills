...
**Out of Scope:** building actual analytics dashboards (BI tools — this produces the spec); statistical experiment design (`reference/brainstorm-experiments/`); financial/revenue forecasting (`finance/`); OKR drafting (`brainstorm-okrs/`); data quality validation (`data-analytics/`).
...
|-------------|-----------|-------------|
| `reference/brainstorm-ideas/` | Receives from | Opportunity discovery defines what value to deliver; NSM measures it |
| `reference/identify-assumptions/` | Receives from | NSM candidates surface assumptions about what customers value |
| `reference/brainstorm-okrs/` | Feeds into | NSM becomes the quarterly Objective; inputs become Key Results |
| `reference/outcome-roadmap/` | Feeds into | Roadmap items justify themselves by which input metric they target |
| `reference/prioritization-frameworks/` | Pairs with | NSM impact is one of the scoring criteria (e.g., RICE Impact, Weighted) |
| `reference/status-update-generator/` | Feeds into | NSM and input movements feature in Highlights of weekly updates |
