...
**Out of Scope:** Live incident command and on-call coordination (see `delivery-manager/`). Sprint retrospectives on team practice (see `sprint-retrospective/`). Risk surfacing before launch (see `reference/pre-mortem/`). Quantitative reliability engineering and error-budget policy (see `engineering/` skills if present).
...
|---|---|---|
| `reference/pre-mortem/` | Bidirectional | Post-mortem findings update next launch's pre-mortem risk register; pre-mortem mitigations become post-mortem-checked controls |
...
| `reference/dependency-map/` | Receives from | Cross-team contributing factors map to dependency-graph nodes |
| `reference/status-update-generator/` | Feeds into | Sev 0/1 incidents surface in weekly executive status updates |
