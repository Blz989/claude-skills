...
1. Pick one segment; run 5-7 interviews via `reference/customer-interview-script/`.
2. Fill the Customer Profile (jobs/pains/gains, ranked) from `reference/interview-synthesis/` themes; then draft the Value Map so each top item has a pain reliever or gain creator.
3. Run `assets/fit_validation_checklist.md`, list the top unaddressed pains/gains, and feed the canvas into `reference/create-prd/` Sections 5-6.
...
**In scope:** Customer Profile (jobs, pains, gains) construction and ranking; Value Map construction; problem-solution fit validation; mapping the canvas into PRD inputs (`reference/create-prd/` Sections 5 and 6); sales-enablement translation.
...
**Caveats:** the VPC is a thinking aid, not a roadmap -- solutions still need experimentation (`reference/brainstorm-experiments/`). Problem-solution fit is the minimum bar, necessary but not sufficient for product-market fit. Strategyzer methodology is CC-BY-SA; attribute to Strategyzer / Osterwalder when sharing externally. A beautifully filled canvas with no customer interviews is fiction.

## Integration Points

| Integration | Direction | What Flows |
|-------------|-----------|------------|
| `reference/customer-interview-script/` | Receives from | Verbatim customer quotes populate the Customer Profile |
| `reference/interview-synthesis/` | Receives from | Themed insights become jobs, pains, and gains |
| `reference/jtbd-workshop/` | Complementary | JTBD workshop produces the job hierarchy; VPC adds pains and gains |
| `reference/identify-assumptions/` | Bidirectional | Unaddressed pains become risk assumptions; assumptions inform validation focus |
| `reference/create-prd/` | Feeds into | Canvas populates PRD Section 5 (Market Segments) and Section 6 (Value Propositions) |
| `reference/product-vision/` | Bidirectional | Vision defines the long-term promise; VPC validates current-day delivery |
| `reference/prioritization-frameworks/` | Feeds into | Unaddressed top pains and gains become candidate features |
