...
| `reference/create-prd/` | PRD → Linear | PRD page becomes the Linear Project description; PRD sections become Milestones |
| `reference/brainstorm-okrs/` | OKR → Linear | Initiatives mapped to Objectives; Projects tagged with KR identifiers |
| `reference/outcome-roadmap/` | Roadmap → Linear | Outcome roadmap rows mapped to Linear Initiatives; outputs become Projects |
| `reference/release-notes/` | Linear → Release Notes | Closed issues in a cycle/project become release note line items |
| `reference/prioritization-frameworks/` | Scoring → Linear | RICE/WSJF scores written back to labels or description fields |
